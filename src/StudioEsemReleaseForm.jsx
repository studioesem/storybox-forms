...
/* ─────────────────────────────────────────────────────────────
   STUDIO ESEM — Photography & Videography Release (config-driven)

   Served on forms.studioesem.com at /release/<key>. The per-client
   copy comes from `releaseConfig` (see studioEsemReleases.js), so the
   same component serves any client shoot — change the config, not this
   file. Strong defaults below mean it still renders if a field is
   omitted. Matches the Studio ESEM house style (black header, white
   stacked logo, teal links, chartreuse CTA) from StudioEsemICIPForm.
   ───────────────────────────────────────────────────────────── */
...
/* ─── PALETTE — Studio ESEM (matches StudioEsemICIPForm) ─── */
const C = {
  bg:          "#F5F0E8",
  surface:     "#FFFFFF",
  heading:     "#2f2f2f",
...
const ESEM_URL = "https://www.studioesem.com";
...
const DEFAULTS = {
  ...
  definitions: [
    ...
    ["The Production", "is the photography and videography shoot described above, produced by Studio ESEM."],
  ],
  showPaid: true,
  showUnder18: true,
  showCredits: false,
...
/* ─── COMPONENT ─── */
export default function StudioEsemReleaseForm({ releaseConfig, releaseKey }) {
  const R = { ...DEFAULTS, ...(releaseConfig || {}) };
  ...
  const canSubmit = (
    ...
    (!R.showPaid || !paidParticipant || paidAgree) &&
    (!R.showUnder18 || !under18 || parentSignature.length > 0)
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}${R.endpoint}`, { method: "POST", body: fd });
  ...
  const CheckRow = ({ checked, onToggle, children }) => (
    <div
      ...
      style={{
        ...
        background: checked ? "#f7f8dd" : "transparent",
        border: `1.5px solid ${checked ? C.cta : C.border}`,
        transition: "all 0.15s",
      }}
    >
      <div style={{
        width: 24, height: 24, borderRadius: 6, flexShrink: 0,
        border: `2px solid ${checked ? C.cta : C.muted}`,
        background: checked ? C.cta : "transparent",
        display: "flex", alignItems: "center", justifyContent: "center",
      ...
      <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
        <div style={{ maxWidth: 520, margin: "0 auto", padding: "48px 28px", textAlign: "center", fontFamily: body, background: C.surface, borderRadius: 20, boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ marginTop: 24, paddingTop: 20, borderTop: `1px solid ${C.border}`, fontFamily: body, fontSize: "0.8rem", color: C.muted }}>
            <a href={ESEM_URL} target="_blank" rel="noopener noreferrer" style={{ color: C.link, textDecoration: "none", fontWeight: 600 }}>Studio ESEM</a>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Header — black, white stacked logo */}
        <div style={{ background: "#000000", padding: "clamp(24px, 5vw, 32px)", position: "relative" }}>
          ...
          <h1 style={{ fontFamily: heading, fontWeight: 700, fontSize: "clamp(1.45rem, 4vw, 1.9rem)", color: C.cream, margin: "0 0 8px", lineHeight: 1.1, textTransform: "uppercase", letterSpacing: 1 }}>
            {R.title}
          </h1>
          {R.subtitle && (
        ...
        <div style={{ background: C.surface, padding: "clamp(24px, 5vw, 32px)" }}>
          ...
          <section>
            ...
            <ul style={{ margin: "0 0 18px", paddingLeft: 22, fontFamily: body, fontSize: "0.95rem", color: C.body, lineHeight: 1.6 }}>
              {R.clauses.map((clause, i) => (
                <li key={i} style={{ marginBottom: i === R.clauses.length - 1 ? 0 : 8 }}>{clause}</li>
              ))}
            </ul>

            {/* Usage provisions — the client-specific outcome */}
            {R.usageProvisions && (
              <div style={{
                background: "#f7f8dd",
            ...
            <div style={{
              ...
              {R.definitions.map(([term, def], i) => (
                <p key={i} style={{ ...smallText, margin: i === R.definitions.length - 1 ? 0 : "0 0 8px" }}>
          ...
          <section style={sectionWrap}>
            <h3 style={sectionHeading}>Shoot Details</h3>
            ...
            <div style={fieldWrap}>
              ...
              <input
                ...
                placeholder="e.g. Carriageworks, Eveleigh"
          ...
          {/* SECTION 5 — PAID PARTICIPANT (config-gated, conditional) */}
          {R.showPaid && (
            <section style={sectionWrap}>
              <CheckRow checked={paidParticipant} onToggle={() => setPaidParticipant(!paidParticipant)}>
              ...
              {paidParticipant && (
                <div style={{ marginTop: 18 }}>
                  <p style={{ ...smallText, margin: "0 0 14px" }}>
                    Participants who have been engaged for a fee are to send invoices to{" "}
                    <a href="mailto:hello@studioesem.com" style={{ color: C.link, fontWeight: 600 }}>hello@studioesem.com</a>.
                  </p>
                  <CheckRow checked={paidAgree} onToggle={() => setPaidAgree(!paidAgree)}>
                    I agree
                  </CheckRow>
                </div>
              )}
          ...
          {/* SECTION 6 — UNDER 18 (config-gated, conditional) */}
          {R.showUnder18 && (
            <section style={sectionWrap}>
              <CheckRow checked={under18} onToggle={() => setUnder18(!under18)}>
              ...
              {under18 && (
                <div style={{ marginTop: 18 }}>
                  <p style={{ ...smallText, margin: "0 0 18px" }}>
                    If this release is for a participant under the age of 18, the signature of that person's parent or legal guardian is also required.
                  </p>
          ...
          <div style={{ marginTop: 28, paddingTop: 16, borderTop: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontFamily: heading, fontSize: "0.65rem", fontWeight: 700, color: C.muted, letterSpacing: "0.2em", textTransform: "uppercase" }}>
              <a href={ESEM_URL} target="_blank" rel="noopener noreferrer" style={{ color: C.link, textDecoration: "none" }}>Studio ESEM</a>
            </span>
          </div>
        </div>
      </div>
